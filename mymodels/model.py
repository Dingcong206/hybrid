...
# ============================================================
# 7. Residual Bidirectional Mamba Block
# ============================================================
class ResidualBiMambaBlock(nn.Module):
    """
    正向Mamba保持主导作用。

    反向Mamba作为小权重残差辅助：

        scale = sigmoid(logit) * max_scale
    ...
    def __init__(
        ...
        expand: int = 2,
        dropout: float = 0.15,
        backward_max_scale: float = 0.30,
        backward_init_logit: float = -2.0,
    ) -> None:
        super().__init__()
        ...
        if not (
            ...
            raise ValueError(
                "backward_max_scale必须位于[0,1]。"
            )

        self.backward_max_scale = float(
            backward_max_scale
        )
        ...
        if HAS_MAMBA:
            self.forward_sequence_model = Mamba(
                d_model=dim,
                d_state=d_state,
                d_conv=d_conv,
                expand=expand,
            ...
            self.backward_sequence_model = Mamba(
                d_model=dim,
                d_state=d_state,
                d_conv=d_conv,
                expand=expand,
            ...
            self.forward_sequence_model = nn.GRU(
                input_size=dim,
                hidden_size=dim // 2,
                num_layers=1,
                batch_first=True,
            ...
            self.backward_sequence_model = nn.GRU(
                input_size=dim,
                hidden_size=dim // 2,
                num_layers=1,
                batch_first=True,
        ...
        self.backward_scale_logit = nn.Parameter(
            torch.tensor(
                float(
                    backward_init_logit
                ),
    ...
    def get_backward_scale_tensor(
        self,
    ) -> torch.Tensor:
        return (
            torch.sigmoid(
                self.backward_scale_logit
            )
            * self.backward_max_scale
        )

    def get_backward_scale(
        self,
    ) -> float:
        return float(
            self.get_backward_scale_tensor()
            .detach()
            .cpu()
            .item()
        )
    ...
    def forward(
        ...
        # ----------------------------------------------------
        forward_output = (
            self.run_sequence_model(
                self.forward_sequence_model,
                normalized,
            )
        )
...
class TimeFrequencyEncoder(nn.Module):
    ...
    def get_backward_scales(
        self,
    ):
        return [
            block.get_backward_scale()
    ...
    def forward(
        ...
        # ----------------------------------------------------
        # Residual Bidirectional Time-Mamba
        # [B,F,T,D] -> [B*F,T,D]
        # ----------------------------------------------------
        for block in self.time_blocks:
...
# ============================================================
# 11. D4.3 Residual BiMamba Model
# ============================================================
class DTFHybridModel(nn.Module):
    ...
    def get_backward_scales(
        ...
        return self.encoder.get_backward_scales()
...
if __name__ == "__main__":
    ...
    print(
        "D4.3 Residual BiMamba shape test passed."
    )