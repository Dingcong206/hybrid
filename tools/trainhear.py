...
# ============================================================
# 0) 确保能导入 mymodels.model
#    运行方式：python tools/train_hear_tokens.py
# ============================================================
...
class TokenNPYDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        x = np.load(npy_path)               # (T, D) 例如 (200, 1024)
...
def main():
    ...
    parser.add_argument("--root", type=str, required=True,
                        help="包含 train_index.csv / test_index.csv 的目录（你的HeAR token输出目录）")
    ...
    # 模型超参（适配 HeAR tokens: in_dim=1024）
    ...
    ckpt_path = os.path.join(args.save_dir, "best_model_hear_tokens.pt")
    ...
    if args.binary:
        ...
        raise ValueError("当前脚本默认二分类，请使用 --binary（默认已开启）。")

    # ---------- build model ----------
    model = build_model(
        in_dim=args.in_dim, d_model=args.d_model, n_layers=args.n_layers, nhead=args.nhead, dropout=args.dropout
    ).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for x, mask, y in dl_train:
            x = x.to(device, non_blocking=True)
            mask = mask.to(device, non_blocking=True)
            y = y.to(device, non_blocking=True).float()  # BCE expects float
